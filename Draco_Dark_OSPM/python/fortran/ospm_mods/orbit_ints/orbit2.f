...
C       ORBIT2 is for not-edge-on systems (theta < 85)
C
C     USED BY LIBRARY
C
C=============================================================================D
      SUBROUTINE orbit2(pos0,epsilon,iesc,isos,r_a,r_p,xlibT,
...
C get the anisotropy stuff
        ...
        if(ira.le.Nrani.and.ira.ge.1) then
           ...
           if(ita.le.Ntani.and.ita.ge.1) then
              ...
              vtpliba(ira,ita)=vtpliba(ira,ita)+dt*vth*isign*vphi 
           endif
        endif
...
c        time1=time1+dtime(tarr)
        ...
        tmp1=(vr*cv-vth*v)*sini
        tmp2=vphi*sini
        tmp3=(vr*v+vth*cv)*cosi
        zt=r*v*sini
        yt=r*cv
        ...
        do ireflect=1,2
           ...
           do ip = 1,Npdat
              ...
              if (irsky.le.irmax.and.irsky.gt.0) then
                 ...
                 tVc = tmp1*cp-tmp2*sp+tmp3
                 ivel=nint(tVc*vmult+vadd)
                 if(ivel.ge.1.and.ivel.le.Nvel) then
                    v1libT(ivel,irsky,ivsky)=v1libT(ivel,irsky,ivsky)+
     $                   dt
                 endif
c                 irac=IRCneeR(rsky)
c                 ivac=IVCneeV(abs(vsky))
c                 if(ivac.eq.1.and.irac.le.Nrlib) sumv2(irac)=
c     $                sumv2(irac)+tVc*tVc*dt
              endif
           enddo
        enddo
...
c        if (isos.lt.Nsos.or.vr.gt.0.) goto 999
C---- END TIME LOOP
