      ...
      real xp(nmax),yp(nmax),xpl(nmax),ypl(nmax)
      real xp2(nmax),yp2(nmax),xpl2(nmax),ypl2(nmax)
      real rpl2(nmax),rzl2(nmax),rpl(nmax),rzl(nmax),rpo(nmax)
      real deco(nmax),decl(nmax),red(1000),znorm(1000),waveo(nmax)
      real zplot(10),rplot(10),xplot1(10),yplot1(10)
      real xplot3(1000),yplot3(1000),xplot2(10),yplot2(10)
      integer*8 i6,i7
      character a5*20,a6*20,a26*20,a29*20,a30*20,a31*20,a32*20
      character a33*20,a34*20,a35*20,a36*20,a37*20,a39*20,clab*4
      external func,func2
      common/cosmo/ xh,xm,xl,xr
      parameter(cee=2.99e+5)

      data epsabs,epsrel /1.e-10,1.e-5/

      xbin=3.
      open(unit=1,file='cuts',status='old')
      read(1,*) racen
      read(1,*) xmin,xmax,ymin,ymax
      read(1,*) p1o,p2o,d1o,d2o,sno
      read(1,*) p1l,p2l,d1l,d2l,snl
      close(1)
      open(unit=1,file='axes.dat',status='old')
      read(1,*) (zplot(i),i=1,5)
      read(1,*) (rplot(i),i=1,5)
      close(1)

      contlo=-2.
      conthi=13.

      open(unit=1,file='nz.in',status='old')
      nz=0
      do i=1,1000
 ...
 668  continue
      ...
      dec=(d1o+d2o)/2.
      do iplot=1,5
         call cosmoc(zplot(iplot),degtoMpc,dist)
         roff=degtoMpc*(rplot(1)-racen)*cos(dec/57.3)
         xplot1(iplot)=roff
         yplot1(iplot)=dist*cos(roff/dist)
      enddo
      do iplot=1,5
         call cosmoc(zplot(iplot),degtoMpc,dist)
         roff=degtoMpc*(rplot(2)-racen)*cos(dec/57.3)
         xplot2(iplot)=roff
         yplot2(iplot)=dist*cos(roff/dist)
      enddo
      call cosmoc(zplot(2),degtoMpc,dist)
      do i=1,100
         ra=rplot(1)+float(i-1)/float(100-1)*(rplot(2)-rplot(1))
         roff=degtoMpc*(ra-racen)*cos(dec/57.3)
         xplot3(i)=roff
         yplot3(i)=dist*cos(roff/dist)
      enddo
      ...
      nl=0
      do iall=1,1000000
         read(1,*,end=666) x1,x2,x3,x4,x5,i6,i7,x8,x9,x10,x11
         ra=x9
         dec=x10
         wave=x11
         sn=x3
         z=x4
         if(z<0.5) then
            call cosmoc(z,degtoMpc,dist)
            roff=degtoMpc*(ra-racen)*cos(dec/57.3)
            n=n+1
            xp(n)=roff
            yp(n)=dist*cos(roff/dist)
            deco(n)=dec
            waveo(n)=wave
            rpo(n)=dist
         endif
...
c      dhi=52.3
      ...
      idd=0
      open(unit=11,file='out0',status='unknown')
      ...
      if(idd.eq.1) then
         do id=1,nd
            ...
            print *,d1o,d2o
            do i=1,n
               if(deco(i).gt.d1o.and.deco(i).lt.d2o) then
                  n2=n2+1
                  xp2(n2)=xp(i)
                  yp2(n2)=yp(i)
               endif
            ...
            nl2=0
            do i=1,nl
               if(decl(i).gt.d1l.and.decl(i).lt.d2l) then
                  nl2=nl2+1
                  xpl2(nl2)=xpl(i)
                  ypl2(nl2)=ypl(i)
                  rpl2(nl2)=rpl(i)
            ...
            call pgsch(0.3)
            call plotgray(n2,xp2,yp2,xmin,xmax,ymin,ymax,xbin,
     $           rpl2,rzl2,nz,red,znorm)
c            call pgpt(n2,xp2,yp2,17)
c            call pgpt(nl2,xpl2,ypl2,17)
            call pgsch(1.5)
         enddo
      else
         n2=0
...
c         call pglabel("Mpc","Mpc","")
         call pgsch(0.3)
         call plotgray(n2tot,xp2,yp2,xmin,xmax,ymin,ymax,xbin,
     $           rpl2,rzl2,nz,red,znorm)
         call pgsch(1.5)
c         call pgbox('bcnst',0.,0,'bcnst',0.,0)

         call pgline(2,xplot1,yplot1)
         call pgline(2,xplot2,yplot2)
         call pgline(100,xplot3,yplot3)
         do i=3,5
            write(clab(1:4),3001) zplot(i)
            call pgptxt(xplot1(i),yplot1(i),0.,1.15,clab)
         enddo
         dec=(d1o+d2o)/2.
         do i=3,5
            write(clab(1:4),3002) nint(rplot(i))
            z1=zplot(2)*1.03
            call cosmoc(z1,degtoMpc,dist)
            dist=dist*1.02
            roff=degtoMpc*(rplot(i)-racen)*cos(dec/57.3)
            x1=roff
            y1=dist*cos(roff/dist)
            call pgptxt(x1,y1,0.,0.5,clab)
         enddo
c         call pgptxt(400.,dist*0.98,-10.,0.,"RA")
c         call pgptxt(600.,dist*0.4,58.,0.5,"redshift")
         call pgptxt(1400.,dist*0.98,-10.,0.,"RA")
         call pgptxt(2000.,dist*0.4,58.,0.5,"redshift")
 3001    format(f4.2)
 3002    format(i4)
         ...
         nl=100
         do i=1,nl
            xp(i)=xmin+(xmax-xmin)*float(i-1)/float(nl-1)
            yp(i)=dist1*cos(xp(i)/dist1)
         enddo
         call pgsci(2)
         call pgslw(5)
         call pgline(nl,xp,yp)
         do i=1,nl
            yp(i)=dist2*cos(xp(i)/dist2)
         enddo
         call pgsci(3)
         call pgline(nl,xp,yp)

         call pgsci(4)
         call pgarro(0.,2000.,0.,4200.)
         call pgarro(0.,4200.,0.,2000.)
         call pgsci(6)
         call pgarro(0.,6780.,0.,13600.)
         call pgarro(0.,13600.,0.,6780.)

         call pgsci(1)
         call pgptxt(3000.,1000.,0.,0.,"HETDEX")
         call pgptxt(3000.,5000.,0.,0.,"HETDEX")
         call pgsci(4)
         call pgptxt(3000.,3000.,0.,0.,"DESI")
         call pgsci(6)
         call pgptxt(3000.,9000.,0.,0.,"NOTHING!")
         call pgsci(3)
         call pgptxt(-6000.,11700.,0.,0.,"CMB")
         call pgsci(2)
         call pgsch(2.2)
c         call pgptxt(-6400.,13000.,0.,0.,"\(0766)")
c         call pgptxt(-5500.,13100.,0.,0.,"?")


c         call plotgray(nl2,xpl2,ypl2,xmin,xmax,ymin,ymax,xbin)
c         call pgpt(n2,xp2,yp2,17)
c         call pgpt(nl2,xpl2,ypl2,17)
 999     continue
      endif
      ...
      subroutine plotgray(n,x,y,xmin,xmax,ymin,ymax,xbin,
     $           rpl2,rzl2,nz,red,znorm)
      parameter(narr=5000)
      real x(n),y(n),xsum(narr,narr),tr(6),red(nz),znorm(nz)
      real rpl2(n),rzl2(n)
      ...
      do i=1,narr
         ...
         enddo
      enddo
      nbin=nint((xmax-xmin)/xbin)
      na=nbin

      vmax=0
      do i=1,n
         ix=nint((x(i)-xmin)/xbin)
         iy=nint((y(i)-ymin)/xbin)
         if(ix.ge.1.and.ix.le.narr.and.iy.ge.1.and.iy.le.narr) then
            if(rzl2(i).gt.9.9) then
c            if(rzl2(i).gt.1.9) then
c               call xlinint(rzl2(i),nz,red,znorm,sumnorm)
c               isp=nint(1./sumnorm)
               isp=5
               if(isp.gt.1) then
                  ilo=max(1,ix-isp)
                  ihi=min(narr,ix+isp)
                  ...
                  do is=ilo,ihi
                     do js=jlo,jhi
                        xsum(is,js)=xsum(is,js)+1./100.
                     enddo
                  enddo
               else
               xsum(ix,iy)=xsum(ix,iy)+1.
            endif
           else
               xsum(ix,iy)=xsum(ix,iy)+1.
            endif
c            xsum(ix,iy)=xsum(ix,iy)+1
            vmax=max(vmax,xsum(ix,iy))
         endif
      enddo
      print *,na,vmax
      ...
      do i=1,narr
         do j=1,2500
c         do j=1,narr
c            xsum(i,j)=(xsum(i,j))**0.2
            xsum(i,j)=(xsum(i,j))**0.5
         enddo
      enddo
      vup=1.1
      vlo=0.
      ...
      call pggray(xsum,narr,narr,1,na,1,na,vup,vlo,tr)

      return
      end

      subroutine cosmoc(z,degtoMpc,dist)
      ...
      call qags(func,0.,z,epsabs,epsrel,ss,abserr,
     $     neval,ier,limit,lenw,last,iwork,work)

      call qags(func2,0.,z,epsabs,epsrel,ss2,abserr,
     $     neval,ier,limit,lenw,last,iwork,work)

      dh=cee/xh
      dc=dh*ss2
      if(xr.gt.0) then
         dm=dh/sqrt(xr)*sinh(sqrt(xr)*dc/dh)
      elseif(xr.lt.0) then
         dm=dh/sqrt(-xr)*sin(sqrt(-xr)*dc/dh)
      else
         dm=dc
      endif

      xnum=2.*(2.-xm*(1.-z)-(2.-xm)*sqrt(1.+xm*z))
      den=xm*xm*(1.+z)
      d2=cee/xh*xnum/den

      degtoMpc=dm/206265.*60.*60.
      dist=dm
c      print *,'Time (lookback), Radius (kpc/asec) : ',
c     $     ss*9.76e2/xh,dm/206265.*1000./(1.+z)
c      print *,'Distance Mpc (trans comoving, proper), ang diameter : ',
c     $     dm,dm/(1.+z)
c      print *,'Comoving Mpc/arcmin : ',dm/206265.*60.
c      dml=dm*(1.+z)*3.09e22
c      dconv=4.*3.14*dml*1.e-17/1.e20
c      dconv=dconv*dml*1.e4/1.e20
c      print *,"D_L (lum dist, in meters) = ",dml,dconv
      ...
      common/cosmo/ xh,xm,xl,xr
      xp=1.+x
      den=sqrt(xm*xp*xp*xp+xr*xp*xp+xl)
      func=1./den/xp
      return
      end
