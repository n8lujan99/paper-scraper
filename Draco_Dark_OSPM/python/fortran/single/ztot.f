...
c      xr=0.
 ...
 1    write(*,"('Input z : '$)")
     ...
     $     neval,ier,limit,lenw,last,iwork,work)
      ...
      den=xm*xm*(1.+z)
      d2=cee/xh*xnum/den
c      xnum=z*(1.+sqrt(1.+xm*z)+z)
c      den=(1.+z)*(1.+sqrt(1.+xm*z)+xm*z/2.)
c     d3=cee/xh*xnum/den
      print *,'Time (lookback), Radius (kpc/asec) : ',
     $     ss*9.76e2/xh,dm/206265.*1000./(1.+z)
      print *,'Distance Mpc (trans comoving, proper), ang diameter : ',
     $     dm,dm/(1.+z)
      print *,'Comoving Mpc/arcmin : ',dm/206265.*60.
      dml=dm*(1.+z)*3.09e22
      dconv=4.*3.14*dml*1.e-17/1.e20
      dconv=dconv*dml*1.e4/1.e20
      print *,"D_L (lum dist, in meters) = ",dml,dconv
      if(nz.eq.1) then
         vol=(dm/206265.*60.)**2*(dm-dmold)
         print *,'Comoving distance,vol from previous : ',dm-dmold,vol
      endif
      dmold=dm
      nz=1
      goto 1
