      ...
      real ao(nca),xin(nmax),xoutv(nca),xouts(nca)
      ...
      n=nint(wavec*2./2.)
      do i=1,n
         wave(i)=wave0-wavec+2.*float(i-1)
         flux(i)=0.
         fluxe(i)=fluxerr
      enddo
 666  continue
...
c      idum=-1
      ...
      do isim=1,nsim
         ...
         xnp=4.*sigg
         xnp=xnp/pixsize
         if(abs(wfit-wl0(1)).gt.8.or.amp.le.0.) goto 766
...
c - get noise from the errors
         ...
         ston=ston2
         if(sigg.gt.1.7.and.sigg.lt.6.0) then
            if(ston.gt.4.7.and.ston.lt.13) then
               ng1=ng1+1
               if(ston.gt.5.0) ng2=ng2+1
               if(ston.gt.5.2) ng3=ng3+1
               if(ston.gt.5.4) ng4=ng4+1
               if(ston.gt.5.6) ng5=ng5+1
               xg1=float(ng1)/float(isim)*xres
               xg2=float(ng2)/float(isim)*xres
               xg3=float(ng3)/float(isim)*xres
               xg4=float(ng4)/float(isim)*xres
               xg5=float(ng5)/float(isim)*xres
               write(*,1001) xg1,xg2,xg3,xg4,xg5,isim,chi,ston,wfit
            endif
         endif
 766     continue
 ...
 966  continue
      ...
      xg4=float(ng4)/float(isim)*xres
      xg5=float(ng5)/float(isim)*xres
      write(*,1002) "END",xg1,xg2,xg3,xg4,xg5,isim,xres/float(isim)

 1001 format(5(1x,f7.4),1x,i10,2(1x,f7.4),1x,f8.2)
 1002 format(a3,5(1x,f7.4),1x,i10,1(1x,f7.4))
