c -- makes a lowess fit to data
      ...
      real x(nmax),y(nmax),ys(nmax),rw(nmax),res(nmax),yerr(nmax)
      real yin(nmax),xl(2),yl(2),wksp(nmax),xr(nmax),yr(nmax)
      integer iwksp(nmax),ip(nmax)
      character filein*30,title*40

      data vdlow,big /0.001,1.e10/

 10   call qc1('Velocity data ','dlowess.def',filein)
      open(unit=1,file=filein,status='old',err=10)

      call qr2('Input the min and max radius ','dlowess.def',rlow,rhigh)
      call qr1('Input mean velocity ','dlowess.def',vel)
      call qi1('Do you want vel(0) or abs dev(1) or v^2(2) ',
     $     'dlowess.def',ivel)
      call qr1('Input F ','dlowess.def',f)
      call savdef
...
c         read(1,*,end=666) x1,x2,x3,x4,x5,x6,x7,i8
         ...
         if(rad.ge.rlow.and.rad.le.rhigh) then
            ...
            x(n)=rad
c            x(n)=log10(rad)
            if(ivel.eq.2) y(n)=(x3-vel)*(x3-vel)
            if(ivel.eq.1) y(n)=abs(x3-vel)
            if(ivel.eq.0) y(n)=x3
            yerr(n)=x4
            if(ivel.eq.2) yerr(n)=x4*x4
            ip(n)=17
c            if(i8.eq.577.or.i8.eq.251) ip(n)=23
            xmin=min(xmin,x(n))
            xmax=max(xmax,x(n))
            ymin=min(ymin,y(n))
            ymax=max(ymax,y(n))
 ...
 666  continue
      ...
      call sort3(n,x,y,yerr,wksp,iwksp)

      if(ivel.eq.2) then
         do i=1,n
            yin(i)=y(i)-yerr(i)
         enddo
         call lowess2(x,yin,yerr,n,f,1,0.,ys,rw,res)
      else
         call lowess(x,y,n,f,2,0.,ys,rw,res)
      endif
...
c - look for the rotation subtracted dispersion

      ir=0
      open(unit=1,file='dlowess.rot',status='old',err=101)
      nr=0
      do i=1,nmax
         read(1,*,end=686) x1,x2
         nr=nr+1
         xr(nr)=x1
         yr(nr)=x2
      enddo
 ...
 101  close(1)
      ...
      call qc1('Plot Title ','dlowess.def',title)
      call savdef
      ...
      if(ivel.eq.1.or.ivel.eq.2) call pgwindow(xmin,xmax,-1.,ymax,0,0)
      if(ivel.eq.0) call pgwindow(xmin,xmax,ymin,ymax,0,0)
c      call pgbox('bclnst',0.,0,'bcnst',0.,0)
      call pgbox('bcnst',0.,0,'bcnst',0.,0)
      call pglabel('R (ARCMIN)','','')
      call pglabel('','',title)
      if(ivel.eq.1.or.ivel.eq.2) call pglabel('',
     $     '|\\gDV| (km s\\U-1\\D)','')
      if(ivel.eq.0) call pglabel('','V (km s\\U-1\\D)','')
      call pgsch(.5)
      do i=1,n
         call pgpoint(1,x(i),y(i),ip(i))
      enddo
      call pgsch(1.25)
      call pgline(n,x,ys)
      if(ivel.eq.0) then
         ...
         do i=1,n
            call pgsch(.3)
c            call pgslw(1)
            call pgerry(1,x(i),y(i)+yerr(i),y(i)-yerr(i),1.)
            call pgsch(1.25)
         enddo
      endif
      ...
      if(ir.eq.1) then
         call pgsls(2)
         call pgline(nr,xr,yr)
      endif

      open(unit=2,file='dlowess.out',status='unknown')
      do i=1,n
         write(2,*) x(i),ys(i)
      enddo
      close(2)
