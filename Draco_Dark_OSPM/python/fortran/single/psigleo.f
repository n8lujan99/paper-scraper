...
c      open(unit=1,file='sigma.dat',status='old')
      open(unit=1,file='sigin',status='old')
      ...
      do i=1,nmax
         ...
         yp2s=10**yp2s
c         signew=((yp2s+fcon2)*x2*x2-fcon2*sigc2*sigc2)/(yp2s+fcon2)
         signew=((yp2s+fcon2)*x2*x2-fcon2*sigc2*sigc2)/(yp2s)
         signew=sqrt(max(0.,signew))
         sig2(ns)=signew
         if(ns.eq.1) sig20=signew/x2
         sig2(ns)=sig2(ns)/sig20

         yp3s=10**yp3s
c         signew=((yp3s+fcon3)*x2*x2-fcon3*sigc3*sigc3)/(yp3s+fcon3)
         signew=((yp3s+fcon3)*x2*x2-fcon3*sigc3*sigc3)/(yp3s)
         signew=sqrt(max(0.,signew))
         sig3(ns)=signew
         if(ns.eq.1) sig30=signew/x2
         sig3(ns)=sig3(ns)/sig30
         write(11,*) x1,x2,sig2(ns),sig3(ns)
      enddo
 667  continue
      close(1)
      close(11)
