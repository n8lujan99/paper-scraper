...
c      fcon=5.6
      ...
      do i=1,nmax
         ...
         if(x1.gt.rlow) then
c         fcheck=10**(-0.4*(x2-sun))*(20626.5)**2 - fcon
         fcheck=1.
         if(fcheck.le.0) then
            n=n+1
            rl(n)=log10(x1)
...
c         f(n)=log10(10**(-0.4*(x2-sun))*(20626.5)**2 - fcon)
         f(n)=log10(x2)
c         f(n)=x2
c         f(n)=x2-2.*log10(astopc)
         endif
      enddo
 666  continue
...
c      call pglabel('R (arcsec)','\gS (L\D\(2281)\U/asec\U2\D)','')
...
c - plot the mass
      ...
      ymin=ymin-ybit
      ymax=ymax+ybit
      if(abs(ymin-ymax).le.0.5) then
c      if(ymin.ge.ymax) then
         ymin=mass(1)-0.25
         ymax=mass(1)+0.25
      endif
...
c estimate log slope of p(r) at large radii to find starting value of p(r)
      ...
      if(gg.le.1.2) print *,'warning: outside pressure nearly divergent'
      yt(2)=-y1*x1/(gg-1.)
      vsig(np)=yt(2)
      do j=1,np-1
         x1=10**rfit(np+1-j)
