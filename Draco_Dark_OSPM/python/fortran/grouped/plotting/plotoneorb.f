...
c      call pgline(2,xl,yl)
 ...
 866  close(11)
      ...
      open(unit=11,file='legend.dat',status='old',err=867)
      read(11,*) xsize,yloff
      call pgsch(xsize)
      do i=1,100
         read(11,*,err=867,end=867) c1,xleg,yleg,itype,icol
         call pgsci(icol)
         call pgptxt(xleg,yleg,0.,-0.1,c1)
         call pgpt1(xleg,yleg+yloff,itype)
      enddo
 867  continue
      close(11)

      open(unit=1,file='smline.out',status='old',err=889)
      n=0
      do i=1,10000
         read(1,*,end=888,err=889) x1,x2
         n=n+1
         xl(n)=x1
         yl(n)=x2
      enddo
