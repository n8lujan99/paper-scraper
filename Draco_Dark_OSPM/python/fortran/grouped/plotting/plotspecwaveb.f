      ...
      real siga(nmax),xisig(nmax),xin3(nmax)
...
c      ibin=1
      ...
      open(unit=1,file='out',status='old',err=566)
      id=0
      do i=1,nmax
         read(1,*,end=566) cname
         id=id+1
         cdone(id)=cname
      enddo
 566  continue
...
c      ymax=1.
     ...
     $        '10\U-17\D ergs/cm\U2\D/s/\(2078)','')
      ...
      call pgmtxt('T',-1.5,0.05,0.0,"highest")
      call pgsch(2.0)
      call pgmtxt('T',1.,0.98,0.0,file1(22:31))
      call pgsch(1.5)
c      call pgslw(4)
      call pgslw(2)
      ic=0
...
c         xin2(k)=sum/float(np)
     ...
     $        '10\U-17\D ergs/cm\U2\D/s/\(2078)','')
      ...
      call pgmtxt('T',-1.5,0.05,0.0,"lowest")
      call pgslw(2)
      ic=0
      do j=1,np
         nft=nint(xisum(j))
      ...
      do k=1,n
         ...
         enddo
c         call biwgt(xin1,np,xb1,xs1)
         xin2(k)=sum/float(nsum1)
      enddo
      call pgslw(8)
      call pgsci(1)
      call pgline(n,x,xin2)
      call getstat(n,x,xin2,wave,sig2,xs2)
      call pgslw(2)
      call pgslw(2)
      call pgsci(1)
      call pgenv(xmin,xmax,ymin,ymax,0,0)
     ...
     $        '10\U-17\D ergs/cm\U2\D/s/\(2078)','')
      ...
      call pgmtxt('T',-1.5,0.05,0.0,"high")
      jhalf=nint(float(nfibt)/2.)
      ic=0
      do j=jhalf,nfibt-np
         nft=nint(xisum(j))
         do k=1,n
     ...
     $        '10\U-17\D ergs/cm\U2\D/s/\(2078)','')
      ...
      call pgmtxt('T',-1.5,0.05,0.0,"low")
      ic=0
      do j=np+1,jhalf-1
         nft=nint(xisum(j))
         do k=1,n
      ...
      igood=0
      if(sig1.gt.-1.4*sig2) igood=1
      write(*,1001) iall,cname,sn,sig1,sig2,igood,xs1,xs2
c      read *,igood
 
      open(unit=11,file='out',status='unknown',position='append')
      write(11,1002) cname,wave,sn,igood,sig1,sig2,xs1,xs2
      close(11)
