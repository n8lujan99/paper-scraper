
      parameter(nmax=10000)
      real x(nmax),y(nmax),xn(nmax),yn(nmax),suma(200),xsum(200)
      real xb(nmax),yb(nmax),xin(20),xine(20),yin(nmax,200)
      real xisum(200),xin1(nmax),xin2(nmax),yin2(nmax,200)
      real rada2(nmax)
      real siga(nmax),xisig(nmax),xin3(nmax),rada(200),xwa(200)
      character cdone(nmax)*10,a5*24,a6*5,a9*20
      character file1*80,file2*80,c1*18,afib(nmax)*12,cname*10

      radcut=0.9
      idumb=1 ! 0 is normal, 1 if per A

      wavec=30.
      wc=3.5
      wc=4.5
      nf=18
      ibin=7
c      ibin=1
      ib1=(ibin-1)/2
      xib=float(ibin)

      call pgbegin(0,'?',2,2)
c      call pgbegin(0,'?',1,1)
      call pgpap(0.,1.)
      call pgsch(1.5)
      call pgscf(2)
      call pgslw(3)
c      call pgask(.false.)

      xmin=3500.
      xmax=5500.

      open(unit=3,file='splistaf',status='old')

      do iall=1,100000
c      do iall=1,1

      read(3,*,end=878) cname,wave0,sn
      file1=cname//".splista"

c      open(unit=1,file='splista',status='old')
      open(unit=1,file=file1,status='old')

      nl=0
      ic=0
      nfibt=0
      ymin=1e10
      ymax=-ymin
      do il=1,20000
         read(1,*,end=666) file1,wave
c         if(file1(8:19).eq.'20180415v016') goto 966
c         if(file1(8:19).eq.'20180222v020') goto 966
c         if(file1(8:19).eq.'20180508v015') goto 966
         file2=file1
         file2(33:37)="list "
         xmin=wave-wavec
         xmax=wave+wavec
         open(unit=2,file=file1,status='old',err=888)
         open(unit=7,file=file2,status='old',err=888)
         goto 889
 888     continue
         print *,"Does not exist: ",file1
         goto 667
 889     continue
         read(2,*) ifib
         do i=1,ifib
            read(7,*) x1,x2,x3,x4,a5,a6,rad,x8,a9,i10,i11,i12,i13,xw
            rada(i)=rad
            xwa(i)=xw
         enddo
         close(7)
         nfibt=nfibt+ifib
         n=0
         x10=0.
         do i=1,nmax
            read(2,*,end=667) x1,(xin(j),xine(j),j=1,ifib)
            if(x1.gt.xmin.and.x1.lt.xmax) then
               n=n+1
               x(n)=x1
               do j=1,ifib
                  ntmp=nfibt-ifib+j
                  yin(n,ntmp)=xin(j)
                  rada2(ntmp)=rada(j)
                  afib(ntmp)=file1(8:19)
                  ymin=min(ymin,xin(j))
                  ymax=max(ymax,xin(j))
               enddo
            endif
         enddo
 667     continue
         close(2)
 966     continue
      enddo
 666  continue
      close(1)

      ic=0
      xcmin=wave-wc
      xcmax=wave+wc
      do j=1,nfibt
         sum=0.
         sumf=0.
         nsum=0
         nsumf=0
         do k=1,n
            y(k)=yin(k,j)
            if(x(k).gt.xcmin.and.x(k).lt.xcmax) then
               sum=sum+y(k)
               nsum=nsum+1
               xin1(nsum)=y(k)
            else
               sumf=sumf+y(k)
               nsumf=nsumf+1
               xin2(nsumf)=y(k)
            endif
         enddo
         call biwgt(xin1,nsum,xb1,xs1)
         call biwgt(xin2,nsumf,xb2,xs2)
         suma(j)=sum
         xsum(j)=suma(j)
         xisum(j)=float(j)
         siga(j)=xs2
         xin3(j)=xs2
         xisig(j)=float(j)
      enddo

      call biwgt(xin3,nfibt,xbsig,xssig)
      xscut=xbsig+4.*xssig

      call sort2(nfibt,xsum,xisum)

      call biwgt(xsum,nfibt,xbs,xss)

      call pgsls(1)
      call pgslw(2)
      call pgsci(1)
      ybit=(ymax-ymin)/30.
      ymin=ymin-ybit
      ymax=ymax+ybit
      ymax=min(2.7,ymax)
      if(idumb.eq.1) ymax=min(0.8,ymax)
      ymin=max(-0.5,ymin)

      call pgsch(1.5)
      call pgenv(xmin,xmax,ymin,ymax,0,0)
      if(idumb.eq.0) then
         call pglabel('Wavelength (\(2078))',
     $        '10\U-17\D ergs/cm\U2\D/s','')
      else
         call pglabel('Wavelength (\(2078))',
     $        '10\U-17\D ergs/cm\U2\D/s/\(2078)','')
      endif
      call pgsch(1.5)
      call pgslw(1)
      call pgsci(14)
      ic=1
      nt=0
      do j=1,nfibt
         if(rada2(j).lt.radcut) then
            nft=nint(xisum(nfibt-j+1))
c            if(siga(nft).lt.0.17) then
            if(siga(nft).lt.xscut/1.5.and.siga(nft).gt.0.1) then
               print *,j,nft,siga(nft),suma(nft),rada2(j),afib(j)
               nt=nt+1
               do k=1,n
                  y(k)=yin(k,nft)
                  if(idumb.eq.1) y(k)=y(k)/2.
                  yin2(nt,k)=y(k)
               enddo
               ic=ic+1
               call pgsci(ic)
               call pgline(n,x,y)
            endif
         endif
      enddo

      do k=1,n
         sum=0.
         do j=1,nt
            xin1(j)=yin2(j,k)
            sum=sum+yin2(j,k)
         enddo
         sum=sum/float(nt)
         call biwgt(xin1,nt,xb1,xs1)
         y(k)=xb1
         y(k)=sum
      enddo
      call pgsci(1)
      call pgslw(5)
c      call pgline(n,x,y)

      enddo
 878  continue
      close(3)

      call pgend
 1001 format(i5,1x,a10,3(1x,f8.2),1x,i1)
 1002 format(a10,1x,f7.2,1x,f7.2,1x,i2,2(1x,f8.2))

      end

      subroutine getstat(n,x,y,w,sig)
      real x(n),y(n),xin(10000)

      wc1=4.5
      wc2=8.0
      w1l=w-wc1
      w1h=w+wc1
      w2l=w-wc2
      w2h=w+wc2

      sum=0.
      ns=0
      do i=1,n
         if(x(i).ge.w1l.and.x(i).le.w1h) then
            sum=sum+y(i)
         endif
         if(x(i).le.w2l.or.x(i).ge.w2h) then
            ns=ns+1
            xin(ns)=y(i)
         endif
      enddo
      call biwgt(xin,ns,xb,xs)
      sig=(sum-xb)/xs
      
      return
      end
