      ...
      real xouta(3),xsum(narrm,narrm),arr2(narrm,narrm),xdo(1032,1032)
      ...
      real arro(narrm,narrm),resmap(1032,112)
      integer naxes(2),ifib(112),ipeaks(10),ifibc(112)
      character file1*40,cspec*100,cback(10)*62,cspecid*3,ccid*15
...
c      ncback=47
      ncback=50
c      cback(1)=
c     $"/scratch/projects/hetdex/lib_calib/back/"//cmth//"sci/"
c      cback(2)=
c     $"/scratch/projects/hetdex/lib_calib/back/"//cmth//"bri/"
c      cback(3)=
c     $"/scratch/projects/hetdex/lib_calib/back/"//cmth//"twi/"
      cback(1)=
     $"/scratch/projects/hetdex/lib_calib/back/100000sci/"
      cback(2)=
     $"/scratch/projects/hetdex/lib_calib/back/100000bri/"
      cback(3)=
     $"/scratch/projects/hetdex/lib_calib/back/100000twi/"
...
c- get data frame
      ...
      do i=1,ncol
         ...
         enddo
      enddo
      sumc=sumc/float(nrow*ncol)
      if(sumc.lt.1.) goto 706
 ...
 1003 format(i3)
      ...
      call ftgkye(im1,'RDNOISE',xnoise,file1,ier)
      if(ierr.ne.0) then
         xnoise=3.0
         ier=0
      endif
      if(xnoise.gt.2.0.and.xnoise.lt.4.0) then
      else
         xnoise=3.0
      endif
      call ftclos(im1,ier)
...
c- get pixel flat
      do i=1,1032
         ...
         enddo
      enddo
c      cspec="/work/projects/hetdex/maverick/virus_config/lib_pflat/"//
c     $     "pixelflat_cam"//cspecid//"_"//cpos//chalf//".fits"
      cspec="/scratch/projects/hetdex/lib_calib/lib_pflat/"//
     $     "pixelflat_cam"//cspecid//"_"//cpos//chalf//".fits"
      call getifits(cspec,xflat,ier)
      close(1)
      do i=1,1032
         do j=1,1032
...
c- fix for the 516-517 col
      ...
      do i=516,517
         do j=200,800
            if(xflat(i,j).ne.0) ifixcol=0
         enddo
      enddo
      if(ifixcol.eq.1) then
         do i=516,517
            do j=1,1032
               if(iuse.eq.0) then
                  xflat2(i,j)=1.
            ...
            xin(1)=xd(i,1)
            do j=2,1032
               xin(j)=ysh*xd(i,j-1)+(1.-ysh)*xd(i,j)
            enddo
            do j=1,1032
               if(iuse.eq.0) then
                  xdo(i,j)=xin(j)*fsh
...
c- get bias
      do i=1,narrm
         ...
         enddo
      enddo
      cspec="/scratch/projects/hetdex/lib_calib/lib_mbias/"
     $     //cmth//"/masterbias_"//cspecid//"_"//ccid(6:8)//
     $     "_"//cpos//chalf//".fits"
c      cspec="/scratch/projects/hetdex/lib_calib/lib_mbias/"
c     $     //"202200"//"/masterbias_"//cspecid//"_"//ccid(6:8)//
c     $     "_"//cpos//chalf//".fits"
      call getifits(cspec,xbias,ier)
      if(ier.ne.0) then
         cspec="/scratch/projects/hetdex/lib_calib/lib_mbias/"
     $        //"202200"//"/masterbias_"//cspecid//"_"//ccid(6:8)//
     $        "_"//cpos//chalf//".fits"
         call getifits(cspec,xbias,ier)
      endif
...
c- remove overscan and bias, and divide by flat
      ...
      do j=iyt1,iyt2
         ...
         do i=ixt1,ixt2
            ...
            arro(nx,ny)=xdo(i,j)-xb-xbias(i,j)
...
c            if(xflat(i,j).gt.0) then
            ...
            if(xflat2(i,j).gt.0.666) then
               arro(nx,ny)=arro(nx,ny)/xflat2(i,j)
            else
               arro(nx,ny)=0.
            endif
         enddo
...
c- get background data
      ...
      do ia=1,3
         cspec=cback(ia)
c         cspec=cspec(1:ncback)//"bk"//cmth//"i"//
         cspec=cspec(1:ncback)//"bk"//
...
c- subtract background and multiply by gain
      do i=1,ncol
         do j=1,nrow
            ...
            if(arro(i,j).ne.0) arro(i,j)=xgain*(arro(i,j)-xbn*xsum(i,j))
         enddo
      enddo
...
c- get the residual map
      call getresid(resmap,cspecid,amp,iuse)

c- subtract the master spectrum
      call subtract(arr,xspec,xtrace,nfbin,x,xprof,xftf,wtrace,
     $     resmap,nmaster,wmaster,xmaster,arrs,arrm,xspecs,xspecsm,
     $     ifib,0)

c- subtract a smooth background from the extracted array
      call getback2(xspecs,ifib,ifibc,ncut,cifupos,amp,iback,iuse,0)
      if(iback.eq.1) call getback(xspecs,ifib,ifibc,ncut,iuse,0)
...
c- get the error frame and count zeros
      call geterror(arrm,arrs,xspecsm,xflat2,xtrace,nfbin,x,
     $     xprof,ifib,arrerr,specerr,xnoise)
...
c- extract the spectra for the twilight with col fix
      if(iuse.eq.0) then
         call extract(arro,xtrace,nfbin,x,xprof,xspec,xspecr,xspecc,
     $     iignore,ifib,xnoise,iuse)
...
c         call ftpkls(51,'EXTNAME','error_analysis',"Label",ier)
 ...
 706  continue
      ...
      subroutine getresid(resmap,cspecid,amp,iuse)
      real resmap(1032,112)
      integer naxes(2)
      character cspecid*3,amp*2,file1*120
      logical simple,extend,anyf

      if(iuse.eq.0) return
      
      file1="/scratch/projects/hetdex/lib_calib/reschi/res"
     $     //cspecid//amp//".fits"
      ier=0
      iread=0
      ...
      if(ier.eq.0) then
         ...
         ncol=naxes(1)
         nrow=max(1,naxes(2))
         call ftg2de(52,igc,0.,1032,ncol,nrow,resmap,anyf,ier)
         call ftclos(52,ier)
      else
         ier=0
         do j=1,112
      ...
      subroutine getifits(cspec,xbias,ier)
      parameter(narrm=2000)
      ...
      call ftclos(im1,ier)
 706  continue
c      if(ier.ne.0) print *,"No image for: ",cspec
...
c- get fiber profile
      if(icalloc.eq.0) then
         file1="/work/projects/hetdex/maverick/virus_config/lib_calib/"
...
c      print *,"Reading: ",file1
      ...
      goto 711
 710  continue
c      print *,"No wavelength solution for: ",file1
 711  continue
      close(1)
...
c- copy results over
      ...
      real xspecn(narrm,112),xla(3),xba(3),xplot(112),yplot(112)
...
c            if(ifibc2(j).eq.1) xspecs(i,j)=0.
      ...
      subroutine getback2(xspecs,ifib,ifibc,ncut,cifupos,amp,iback,
     $     iuse,iplot)
      parameter(narrm=2000)
      real xspecs(narrm,112),xin(1032),xntry(10)
      real xin1(1032*10),xin2(1032*10),xin3(1032*10),xin5i(112)
      real xspecn(narrm,112),xla(3),xba(3),xin4(1032),xin5(112)
      real xicol(10),xcon(10),xslope(10),xfix(1032,112)
      real xplot(112),yplot(112),yplot2(112),xl(2),yl(2)
      integer ifib(112),ifibc(112),ifibc2(112),ifibnew(112)
...
c         print *,ni,ilo,ihi,xcon(ni),xslope(ni)
      enddo
      ...
      do j=1,112
         do i=1,1032
            if(xspecs(i,j).eq.0) then
               ...
               xspecs(i,j)=xspecs(i,j)-xspecn(i,j)
c               if(i.gt.408.and.i.lt.410) print *,j,i,xspecn(i,j)
            endif
         enddo
      enddo
...
c- find the continuum sources                                                                                                                     
      ...
      do j=1,112
         xcut=xcuta
         if(j.ge.icuta.and.j.le.icutb) xcut=xcutb
         ifibc(j)=0
         ifibc2(j)=0
         xcheck=xin3(j)+xb
         if(xcheck.gt.xcut) ifibc(j)=1
...
c - remove +-1 around continuum fiber
      ...
      do j=1,112
         ifibc(j)=ifibnew(j)
         if(ifibc(j).eq.1) ncut2=ncut2+1
c         print *,amp," ",j,ifibc(j)
      enddo
      ncut=ncut2

      if(iplot.eq.1) then
         call pgsci(1)
c         call pgenv(1.,112.,-50.,100.,0,0)
         call pgenv(0.,113.,-10.,20.,0,0)
         do i=1,112
            if(ifib(i).eq.1.or.ifibc(i).eq.1) then
               call pgsci(3)
               xl(1)=float(i)
               xl(2)=float(i)
               yl(1)=-5.
         ...
         call pgline(jplot,xplot,yplot)
         call pgsci(2)
         call pgline(jplot,xplot,yplot2)
         call pgsci(1)
         call pglabel("","",cifupos//amp)
      endif
      ...
      do iy0=1,ny
         ...
         do i=1,n
            ...
            if(i1b.gt.10.) then
               ...
               xin(nin)=abs(val-y(i))
c               print *,i,x(i),y(i),val,xb4,y0,i2b-i1b+1
c               print *,i,n-i1b+1
            endif
c            xin(nin)=sqrt(xin(nin))
         enddo
         call biwgt(xin,nin,xb,xs)
         if(xb.lt.ymin) then
            ymin=xb
...
c            print *,"Best ",y0
      ...
      subroutine geterror(arrm,arrs,xspecsm,xflat2,xtrace,n,x,
     $     xprof,ifib,arrerr,specerr,xnoise)
      parameter(narrm=2000)
      ...
      real xprof(1032,112,15),x(15),xspecsm(1032,112)
      integer ifib(112)
...
c- first get the profile normalization
...
c            err=sqrt(7.*readn*readn+xspecsm(i,j))
c            specerr(i,j)=err
            if(ifib(j).eq.1) then
               specerr(i,j)=0.
            endif
c- check if >=3 of the elements are zero, then remove
     ...
     $     ifib,iplot)
      ...
      real xspecsm(1032,112),arrm(1032,1032),resmap(1032,112)
      ...
      do j=1,112
         ...
         do i=1,1032
            ...
            sky0=sp0*xftf(i,j)
            res0=resmap(i,j)*sky0
            sky0=sky0+res0
            xspecs(i,j)=xspec(i,j)-sky0
         ...
         if(ifib(j).eq.0) then
            ...
            do i=1,1032
               ...
               do k=jlo,jhi
                  ...
                  call xlinint2(xjp,n,x,xin,yjp,jin2,jout2)
                  jin2=jout2
c                  xs=xs0*xftf0*yjp/sum1
                  xs=(xs0+res0)*xftf0*yjp/sum1
                  arrm(i,k)=xs
                  iarr(i,k)=iarr(i,k)+1
...
c      open(unit=3,file="/work/projects/hetdex/maverick/"
c     $     //"virus_config/lib_calib/woff.in",status="old")
      open(unit=3,file=
     $     "/scratch/projects/hetdex/lib_calib/datafiles/woff.in",
     $     status="old")
...
c- get solar and write into ys. Interpolate virus to solar bins into ys2
      open(unit=1,file=
     $     '/scratch/projects/hetdex/lib_calib/datafiles/sun_use.dat',
     $     status='old')
      ns=0
      jin=1
      do i=1,nmax
...
c- don't know why this is here....ignore for now
      if(amp.eq.'LL'.or.amp.eq.'RU') then
         i1a=13
         i2a=112
      endif
...
c            jin=jout
      ...
      open(unit=1,
     $     file="/scratch/projects/hetdex/lib_calib/datafiles/ctrap.all"
     $     ,status='old',err=777)
      do i=1,10000
 ...
 777  continue
      ...
      open(unit=1,
     $file="/scratch/projects/hetdex/lib_calib/datafiles/pdefect.all",
     $     status='old',err=777)
      do iall=1,10000
...
c            call pgsci(1)
     ...
     $     xtrace,ifib,iuse,iplot)
      ...
      parameter (narrm=2000)
      real arr(narrm,narrm),xfib(112),xfiba(1032,112),xoff(15)
...
c- get the default fiber positions
     ...
     $     "_"//cifu//"_"//cpos//chalf//".txt"
      ...
      open(unit=1,file=cfib,status='old',err=668)
      read(1,*)
      do i=1,112
 ...
 667  continue
      goto 669
 668  write(*,*) "fiber_loc does not exist: ",cfib
 669  continue
      close(1)
...
c- first get an overall offset; may need a finer grid
      xoff(1)=-8.
      xoff(2)=-4.
      xoff(3)=0.
      xoff(4)=4.
...
c         j0=nint(xfib(ia))
         ...
         i0=icen
         imin=max(1,i0-ihalf0)
         imax=min(1032,i0+ihalf0)
         do i=imin,imax
            do j=jmin,jmax
         ...
         if(ifib(ia).eq.0.and.sumd.gt.0) then
            ng=ng+1
            xin(ng)=xfib(ia)-xoff(ioff)-xfiba(i0,ia)
         endif
      enddo

      if(ng.lt.10) goto 333

      call biwgt(xin,ng,xb,xs)
      do i=1,ng
         xin(i)=abs(xin(i)-xb)
      enddo
      call sort(ng,xin)
      avg=(xin(ng)+xin(ng-1)+xin(ng-2)+xin(ng-3))/4.
      if(avg.lt.offmin) then
         offmin=avg
         offuse=xoff(ioff)+xb
         osig=xs
      endif
      ...
      print *,"Offset from fiducial: ",cifupos,cpos,chalf,-offuse,osig

      do ia=1,112
         xfib(ia)=xfib(ia)-offuse
         j0=nint(xfib(ia))
         jmin=max(1,j0-jhalf)
         jmax=min(1032,j0+jhalf)
         sum=0.
...
c            if(xinner.le.xouter.or.ibad.eq.1) then
            if(xinner.lt.xouter.or.ibad.eq.1) then
               xfiba(i0,ia)=0.
            else
               xfiba(i0,ia)=sum
...
c- set missing fibers to adjacent value
 ...
 888     continue
      ...
      goto 889
 333  continue
c- all flagged, so set to fiducial
      print *,"Offset from fiducial: ",cifupos,cpos,chalf,0.,0.
      do j=1,112
         do i=1,1032
            xtrace(i,j)=xfib(j)
         enddo
...
c- order is, from bottom to top: LU,LL,RL,RU
      ...
      if(cifu.eq."004") then
c         file1="/work/projects/hetdex/maverick/scripts/back/ifu1.txt"
         file1="/scratch/projects/hetdex/lib_calib/datafiles/ifu1.txt"
      else
c         file1="/work/projects/hetdex/maverick/scripts/back/ifu0.txt"
         file1="/scratch/projects/hetdex/lib_calib/datafiles/ifu0.txt"
      endif
...
c      endif
c - from AIP:
c 0209 10.17       -2.20 L 0210    1.000    swap
c 0210 12.71       -2.20 L 0211    1.000    swap
c 0211 15.25       -2.20 L 0212    1.000    swap
c 0212 17.80       -2.20 L 0214    1.000    swap
c 0213 20.34       -2.20 L 0209    1.000    swap
c 0214 22.88       -2.20 L 0213    1.000    swap
c     check 20190410v026, 2100898094, 206.651993 51.077106, 3905
      endif
