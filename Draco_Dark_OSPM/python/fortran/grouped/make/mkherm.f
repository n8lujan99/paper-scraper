...
c         read(1,*,end=666) x1,x2,x3,x4,x5,x6,x7,x8,x9
...
c         rad=x1
 ...
 1001    format('r00',f4.2)
         ...
         elseif(rad.ge.100..and.rad.lt.1000.) then
            write(file1,1003) nint(rad)
         elseif(rad.ge.1000..and.rad.lt.10000.) then
            write(file1,1004) nint(rad)
         else
            write(file1,1005) nint(rad)
         endif
 888     continue

         do i=1,40
            if(file1(i:i).eq." ") goto 777
            nfile=i
         enddo
 777     continue

         open(unit=2,file=file1(1:nfile)//'herm',status='unknown')
