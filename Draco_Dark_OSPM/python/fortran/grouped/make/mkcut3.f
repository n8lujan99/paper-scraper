      ...
      real xin(14),xin2(4)
      integer is(10000),ie(10000),idatef(100000),ishotf(100000)
      character amp(10000)*20,a1*20,adum2*50,aspecf(100000)*20
 ...
 666  continue
      ...
      open(unit=1,file='false.txt',status='old')
      nf=0
      do i=1,100000
         read(1,*,end=555) file1,aspec
         nf=nf+1
         aspecf(nf)=aspec(1:20)
         read(file1(1:8),1005) idatef(nf)
         read(file1(9:11),1006) ishotf(nf)
      enddo
 555  continue
      close(1)
     ...
     $                 aspec(1:20).eq.amp(j)) then
               ...
               enddo
               do j=1,nf
                  if(idate.eq.idatef(j).and.ishot.eq.ishotf(j).and.
     $                 aspec(1:20).eq.aspecf(j)) then
                     write(12,1104) id
                     goto 888
                  endif
               enddo
