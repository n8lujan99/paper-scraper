      ...
      real ra(nmax),dec(nmax),xra(nmax),xdec(nmax)
      real xsn(nmax),xch(nmax)
      character cdate*8,cshot*3,file1*120,cifu*3,a3*13,a1*3
      character a11*11,a12*12

      diffmin2a=2.0
      diffmin2b=3.5
      open(unit=1,file="infomc2",status='old')
      nd=0
      do i=1,nmax
         read(1,*,end=777) x1,x2,x3,x4
         nd=nd+1
         xra(nd)=x1
         xdec(nd)=x2
         xsn(nd)=x3
...
c- now check if there is a source already
         do j=1,nd
            diff=sqrt(cosd*cosd*(rtry-xra(j))**2+(dtry-xdec(j))**2)
            diff=diff*3600.
            diffmin2=diffmin2a
            if(xsn(j).gt.9.) diffmin2=diffmin2b
            if(diff.lt.diffmin2) then
c               print *,"found source"
               goto 887
            endif
         enddo
         ra1=rtry
