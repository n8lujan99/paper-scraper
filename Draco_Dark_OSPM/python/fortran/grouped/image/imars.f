      ...
      parameter (narrm1=17000,narrm2=17000)
...
c      print *,iext,ihd,ier
 ...
 2    call qi1('Add (1) or Mult (2) ','imars.def',itype)
      if(itype.ne.1.and.itype.ne.2) goto 2
      call qr1('Value ','imars.def',val)
      call savdef

      if(itype.eq.1) then
         do j=1,nrow
            do i=1,ncol
c               if(nint(xd(i,j)).ne.-666) xd(i,j)=xd(i,j)+val
               xd(i,j)=xd(i,j)+val
            enddo
         enddo
