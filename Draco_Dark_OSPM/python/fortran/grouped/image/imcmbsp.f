      ...
      parameter (nmax=10000,narrm=10000)
      real val(nmax,narrm),arr(narrm,narrm),xb(nmax),vala(narrm)
      real tarray(2),xin(narrm)
      real*8 bzero,bscale
 ...
 666  continue
     ...
     $        write(6,"('Doing ',i4,a1,$)") irow,char(13)
         ...
         do i=1,n
            ...
            nin=0
            do icol=20,ncol-20
               if(vala(icol).ne.0) then
                  nin=nin+1
                  xin(nin)=vala(icol)
               endif
            enddo
            call biwgt(xin,ncol,xb1,xs1)
            if(xb1.gt.-5.and.xb1.lt.5) then
               ng=ng+1
               do icol=1,ncol
                  val(ng,icol)=vala(icol)
               enddo
            else
               print *,"no good ",xb1,i
            endif
         ...
         do icol=1,ncol
            nin=0
            do i=1,ng
c               if(nint(val(i,icol)).ne.0) then
