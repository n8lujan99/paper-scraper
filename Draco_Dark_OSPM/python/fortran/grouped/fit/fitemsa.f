      ...
      parameter(nmax=2000,nlmax=1000,nca=17)
      ...
      real ydata(nmax),xin(nmax),xine(nmax)
      real fluxa(nmax,100),fluxea(nmax,100)
      ...
      n=0
      do i=1,nmax
         read(1,*,end=666) x1,(xin(j),xine(j),j=1,ifib)
         n=n+1
         wave(n)=x1
         do j=1,ifib
            fluxa(n,j)=xin(j)
            fluxea(n,j)=xine(j)
            if(fluxa(n,j).eq.0) fluxa(n,j)=-666
         enddo
      enddo
 666  continue
      close(1)
...
c - get noise from the errors
         ...
         enddo
c         print *,'Npix, wave1, wave2, wfit = ',nerr,w1,w2,wfit
         if(nerr.gt.0) then
