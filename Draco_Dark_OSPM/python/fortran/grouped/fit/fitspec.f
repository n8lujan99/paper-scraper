...
c         goto 767
         ...
         call getspecc(ifit1,ra,dec,wa,fa,fea,fa2,fea2,chi2f,
     $        fweight,fweight2,
     $        nwa,ntf,raf,decf,az,cfield,rad0,iexp,inative,ratzero,
     $        w0,ww,ditharr,cdith1,cdith2,cdith3,imatch,
     $        nuniq,waveda,tracea,f3out,f4out,idfib,izeroa,fwhm)
...
c- integrate spectrum to get value for fitting PSF weights
     ...
     $     sflux,sfluxe,xw)
      ...
      nt=0
c      print *,"Number of samplings: ",na,ntf
      t1=0.
      t2=0.
...
c            if(ifit1o.eq.102) nsim=100
     ...
     $           nflim,flimw,flimv,flima,ratnoise,i,st0,xflim)
            time3=etime(tarray)
            t1=t1+time2-time1
            t2=t2+time3-time2
            if(istatus.eq.1.and.
...
c- combine over spatial and spectral pairs
 ...
 801  continue
     ...
     $           intuse,wcen,xw,ntf2,weight,iflag,fadcw,az,chi,amps,
     $           sumrata)
...
c- get change in chi^2 if floating the linewidth
            ...
            call fitspec(nspec,spec,sigin,wcen,wrange,ifit1,ifit1o,
     $           ifit3o,nsim,cfield,ra,dec,nout,fitout,xoutv,xouts,
     $           nflim,flimw,flimv,flima,ratnoise,1,st0,xflim)
            fsig1=fitout(1,3)
            fchi1=fitout(1,6)
            sigin=sig0
            call fitspec(nspec,spec,sigin,wcen,wrange,ifit1,ifit1o,
...
c- sum weighted by the PSF weights
     ...
     $              weight,iflag,iexp,fadcw,az,nspec,spec,sumrata)
            ...
            sigin=sig0
            call fitspec(nspec,spec,sigin,wcen,wrange,ifit1,ifit1o,
     $           ifit3o,nsim,cfield,ra,dec,nout,fitout,xoutv3,xouts3,
     $           nflim,flimw,flimv,flima,ratnoise3,1,sn13,xflim3)
            sn03=xoutv3(5)
            endif
...
c- fa2,fea2 is in flux, fa,fea is in counts
      ...
      if(iwrite.eq.1) then
         ...
         write(11,*) ntf
         do i=1,ncol
            write(11,*) wa(1,i),(fa(j,i)*1e17,fea(j,i)*1e17,j=1,ntf)
         enddo
         close(11)
      endif
 ...
 1005 format(i1)
      ...
      subroutine getspecc(ifit1,ra0,dec0,wa,fa,fea,fa2,fea2,chi2f,
     $     fweight,fweight2,nwa,ntf,ra,dec,az,cfield,rad0,iexp,inative,
     $     ratzero,w0,ww,ditharr,cdith1,cdith2,cdith3,imatch,
     $     nuniq,waveda,tracea,f3out,f4out,idfib,izeroa,fwhm)

      parameter(nmax=1100,nfmax=9000,namax=1036,numax=12)
      parameter (narrm1=1036,narrm2=1344)
      real*8 dx1,dx2,drad,dra,ddec
      real xd(narrm1,narrm2),xd2(narrm1,narrm2),xd3(narrm1,narrm2)
      real rat(narrm2),dect(narrm2),xd5(narrm1,narrm2)
      real wa(nfmax,nmax),fa(nfmax,nmax),fea(nfmax,nmax)
      real fa2(nfmax,nmax),fea2(nfmax,nmax),fweight(nfmax,nmax)
      real ra(nfmax),dec(nfmax),az(nfmax),fweight2(nfmax,nmax)
      real chi2f(nfmax,1036),f3out(nmax,nfmax),f4out(nmax,nfmax)
      ...
      integer imatch(nfmax),idfib(nfmax),izeroa(nfmax,nmax)
      character cdith1(nfmax)*28,cdith2(nfmax)*5,cdith3(nfmax)*20
      character file1*120,camp*2,cexp*2,cdate*8,cshot*3
      character a1*6,a2*28,a3*20,a4*5
...
c- get the dither file and find the fibers
     ...
     $        (dcos(ddec/57.3d0)*(dx1-dra))**2+(dx2-ddec)**2)
         ...
         if(rads.lt.rad0) then
            ...
            ditharr(ntf,3)=x4
            ditharr(ntf,4)=x5
c            file1="/scratch/00115/gebhardt/alldet/cal_out/"//
c     $           cdate//"v"//cshot//a2(6:17)//"_cal.fits"
            file1="in_cal.fits"
            cexp=a4(4:5)
            camp=a2(19:20)
            read(a2(22:24),2001) ifib
            call getirow(cexp,camp,is0)
            irow=is0+ifib-1
     ...
     $           extend,ier)
            ...
            call ftg2de(im1,igc,0.,narrm1,ncol,nrow,xd2,anyf,ier)
            call ftmahd(im1,3,ihd,ier)
            call ftg2de(im1,igc,0.,narrm1,ncol,nrow,xd3,anyf,ier)
            call ftmahd(im1,5,ihd,ier)
            call ftg2de(im1,igc,0.,narrm1,ncol,nrow,xd5,anyf,ier)
            call ftclos(im1,ier)
            do i=1,nw
               wave(i)=3470.+2.*float(i-1)
               fin(1,i)=xd5(i,irow)
               fin(2,i)=xd2(i,irow)
               fin(3,i)=xd3(i,irow)
               if(xd(i,irow).ne.0) then
                  fin(4,i)=xd2(i,irow)*xd3(i,irow)/xd(i,irow)
               else
                  fin(4,i)=0.
               endif
               fin(5,i)=1.
            ...
            call sclean(nw,wave,fin,fout)
            do i=1,nw
               wa(ntf,i)=3470.+2.*float(i-1)
c- fa2,fea2 is in flux, fa,fea is in counts
               fa2(ntf,i)=fout(1,i)*1.0e-17
               fea2(ntf,i)=fout(2,i)*1.0e-17
               fa(ntf,i)=fout(3,i)
               fea(ntf,i)=fout(4,i)
               chi2f(ntf,i)=0.
c               if(xd(j,i).eq.0) chi2f(ntf,j)=6.66
               fweight(ntf,i)=1.
               fweight2(ntf,i)=1.
            enddo
            az(ntf)=az0
            read(cexp,2002) iexp(ntf)
            nwa(ntf)=1036
            ditharr(ntf,1)=ra(ntf)
            ditharr(ntf,2)=dec(ntf)
            ditharr(ntf,3)=0.
...
c      read *
 ...
 2002 format(i2)
      ...
      subroutine getirow(cexp,camp,is0)
      character cexp*2,camp*2
...
c            yp=y(j)+(y(j+1)-y(j))*(xp-x(j))/(x(j+1)-x(j))
     ...
     $     ifit3o,nsim,cfield,ra,dec,nout,fitout,xoutv,xouts,
     $     nflim,flimw,flimv,flima,ratnoise,icheck,ston11o,xn2flo)
