      ...
      real wuse(nmax),tuse(nmax)
      character title*12

c      ifit0=1 ! 5th order poly
c      ifit0=2 ! Robin's fit
      ifit0=3 ! empirical fit
...
c - Robin's coefficients
      ...
      read(1,*) title
      close(1)
c      open(unit=1,file='/scratch/00115/gebhardt/detect/tp/tpavg.dat',
      open(unit=1,file='../../tpavg.dat',
     $     status='old')
      xfac=0.85
      na=0
      do i=1,nmax
...
c      call pglabel('Wavelength','Throughput','')
 ...
 778  continue
      ...
      if(ifit0.eq.3) then
         open(unit=2,file="/scratch/00115/gebhardt/detect/tp_hdr3.use",
     $        status='old')
         nuse=0
         do i=1,100
 ...
 567     continue
      ...
      do i=1,np
         ...
         if(ifit0.eq.2) then
            ...
            enddo
         elseif(ifit0.eq.3) then
            call xlinint(xp(i),nuse,wuse,tuse,fit)
         endif
         ynew(i)=fit
      enddo
      if(ifit0.eq.3) ifit0=2
...
c               yf2=yf2*(1.0+(x(i)-4500)*0.18/2000.)
               yf2=yf2*(1.0+(x(i)-4500)*0./2000.)
               xin(i)=y(i)/yf
               if(iin2.eq.0) then
                  yf2=0.
                  xin2(i)=0.
