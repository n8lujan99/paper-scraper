...
c- get the throughput curve
         ntp=0
         file1="/scratch/projects/hetdex/detect/tp/"//
...
c- get the flux correction
         open(unit=1,file=
     $        "/scratch/projects/hetdex/lib_calib/datafiles/fluxcor.use"
     $        ,status='old')
         nfcor=0
         do i=1,1036
...
c- get the dither file and find the fibers
     ...
     $        cdate//"v"//cshot//".dithall"
         open(unit=1,file=file1,status='old',err=121)
         goto 122
 121     continue
         close(1)
         print *,"No dithall, looking locally "
         file1="local.dithall"
         open(unit=1,file=file1,status='old',err=123)
         goto 122
 123     print *,"No local dithall "
 122     continue
         dra=dble(ra0)
         ddec=dble(dec0)
         ntf=0
     ...
     $           (dcos(ddec/57.3d0)*(dx1-dra))**2+(dx2-ddec)**2)
            ...
            if(rads.lt.rad0) then
               ...
               cdith3(ntf)=a3
c- get the multifits               
               file1a(ntf)="/scratch/03261/polonius/red1/reductions/"
...
c- getting the amp2amp                                                                                                               
 ...
 678  continue
      close(2)
      file2(38:41)="XXXX"
      open(unit=2,file=file2,status='old',err=978)
      write(*,*) "Using Amp Norm XXXX "
      goto 679
 978  continue
      write(*,*) "Amp Norm XXXX does not exist: ",file2
      close(2)
      na=3
      wa(1)=3500.
      fa(1)=1.
