 ...
 100  call qc1('Galaxy Spectrum ','fitlov.def',fileg)
      ...
      call geti(fileg,ncol,nrow,xg)

c      wavem=log(4802.)
c      scale=3.33564095e-5

      n=0
      do i=1,ncol
         n=n+1
         xi=float(n-1)
         wave0(n)=wavem+scale*xi+p2*xi*xi+p3*xi*xi*xi
c         wave0(n)=ee**wave0(n)
         flux0(n)=xg(i,1)
      enddo
c      wavem=4802.
c      scale=0.2

c - interpolate to a linear scale

      ntemp=n*1.2
      ntemp=n*1.
      do i=1,ntemp
         x(i)=wavem+scale*float(i-1)
         call xlinint(x(i),n,wave0,flux0,go(i))
      enddo
c      print *,x(ntemp)
      n=ntemp
...
c      scalet=3.33564095e-5
      ...
      do i=1,nfile
         ...
         do ip=1,ncol
            ...
            xi=float(ntt-1)
            dxi=dble(xi)
            dw=dble(wavemt)+dble(scalet)*dxi+dble(p2)*dxi*dxi+
     $           dble(p3)*dxi*dxi*dxi
            wave0(ntt)=sngl(dw)
c            wave0(ntt)=ee**wave0(ntt)
c            wave0(ntt)=wavemt+scalet*xi+p2*xi*xi+p3*xi*xi*xi
            flux0(ntt)=xg(ip,1)
         enddo
...
c         scaleti=0.2
         do it=1,ntt
c            xt(it)=wavemti+scaleti*float(it-1)
            xt(it)=wavemt+scalet*float(it-1)
            call xlinint(xt(it),ntt,wave0,flux0,t(it,i))
         enddo
      enddo
