      ...
      real xin(nmax),yin(nmax),xl(10000),yl(10000),diff(nmax)
      integer ic(nmax)
...
c            diff(i)=(yv-y(i))
         ...
         do i=1,n
            yv=a+b*(x(i)-xoff)+yoff
            diff(i)=abs(yv-y(i))
            if(diff(i).gt.(xb+2.*xs)) ysig(i)=1e6
c            print *,i,diff(i),xb+2.*xs,ysig(i)
         enddo
         call fitexy(xin,yin,n,xsig,ysig,a,b,siga,sigb,chi2,q)
