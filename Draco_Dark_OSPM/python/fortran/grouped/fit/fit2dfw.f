Complete 
 ...
 666  continue
      ...
      do ia=1,100
         ...
         call getchifib(0.,0.,at,fwt,n,xr,xd,xf,xw,
     $        iflag,da,gausa,an,chi,0)
      ...
      amps=atb
      print *,"Begin:"
      call getchifib(0.,0.,amps,fwt,n,xr,xd,xf,xw,
     $     iflag,da,gausa,an,chi,1)
      ...
      fwe=3.5
c      do i=1,nstep
c         xt=xs+(xe-xs)/float(nstep-1)*float(i-1)
c         do j=1,nstep
c            yt=ys+(ye-ys)/float(nstep-1)*float(j-1)
      do ifw=1,nstep
         fwt=fws+(fwe-fws)/float(nstep-1)*float(ifw-1)
         do ia=1,nstep
            at=as+(ae-as)/float(nstep-1)*float(ia-1)
            call getchifib(xt,yt,at,fwt,n,xr,xd,xf,xw,iflag,da,
     $           gausa,an,chi,0)
            if(chi.lt.chimin) then
               chimin=chi
               xtb=xt
...
c      enddo
      ...
      print *,"End:"
      call getchifib(xtb,ytb,atb,fwtb,n,xr,xd,xf,xw,
     $     iflag,da,gausa,an,chi,1)
      print *,xtb,ytb,xrs0,xds0,atb/amps,fwtb
      xnew=xrs0+xtb/3600./cos(xds0/57.3)
      ynew=xds0+ytb/3600.
      print *,fwtb,atb,atb/amps,chimin
      open(unit=11,file='out3',status='unknown')
      write(11,*) fwtb,atb,atb/amps,chimin
      close(11)
 ...
 1001 format(2(2x,f10.5),1x,f10.2,1x,f5.3,2x,a17,1x,a8,1x,a3,1x,a5,
     $     x,i1,1x,f11.2)
      ...
      subroutine getchifib(xrs,xds,amps,fwt,n,xr,xd,xf,xw,iflag,da,
     $     gausa,an,chi,ip)
