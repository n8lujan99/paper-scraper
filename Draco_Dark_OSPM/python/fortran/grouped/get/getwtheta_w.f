      ...
      real ws(1000),ss(1000),w(nmax),f50(nmax)
      real*8 corr(1000)
      integer*8 na,ncorr(1000)
      character cout*3,fout*7,c1*11,cfield(nmax)*11
 ...
 1001 format(a3)
      ...
      open(unit=1,file='weights',status='old')
      ns=0
      do i=1,nmax
         read(1,*,end=444) c1,x2,x3
         ns=ns+1
         f50(ns)=x2
         cfield(ns)=c1
      enddo
 444  continue
      ...
      n=0
      do i=1,nmax
         read(1,*,end=666) x1,x2,x3,c1
         n=n+1
         r(n)=x1
         d(n)=x2
c         call xlinint(x3,ns,ws,ss,s0)
c         w(n)=s0
         do is=1,ns
            if(c1.eq.cfield(is)) then
               w(n)=f50(is)
               goto 888
            endif
         enddo
...
c               corr(ibin)=corr(ibin)+dble((w(i)+w(j))/2.)
               corr(ibin)=corr(ibin)+dble(w(i))
            endif
         enddo
      enddo
