      ...
      real ws(1000),ss(1000),w(nmax),z(nmax),rd(nmax)
      real work(lenw),r0(10),d0(10)
      real*8 i5,corr(1000),corrdm(1000),corrdh(1000)
      integer iwork(limit)
      integer*8 na,ncorr(1000),nrcorr(1000)
      character cout*4,fout*8
      parameter(pi=3.141593e0,radtodeg=57.29578,xlya=1215.666)
...
c      rd2=250.
      ...
      do i=1,nbin
         ...
         rd(i)=rd1+(rd2-rd1)*float(i-1)/float(nbin-1)
         bs(i)=b(i)*b(i)
         xi(i)=float(i)
         ncorr(i)=0
         nrcorr(i)=0
      ...
      bmin=max(0.,bmin)
      bsmin=bmin*bmin
      rdmin=rd(1)-(rd(2)-rd(1))/2.
      rdmin=max(0.,rdmin)

      bmax=b(nbin)+(b(nbin)-b(nbin-1))/2.
      bsmax=bmax*bmax
      rdmax=rd(nbin)+(rd(nbin)-rd(nbin-1))/2.

      open(unit=1,file='data.in',status='old')

      n=0
      do i=1,nmax
         read(1,*,end=666) x1,x2,x3,x4,i5,x6,x7,x8
...
c     do i=1,n
      ...
      do i=i1,i2
         ...
         dfield=1e10
         do ifield=1,4
            df0=(r1-r0(ifield))**2+(d1-d0(ifield))**2
            if(df0.lt.dfield) then
               dfield=df0
               iuse=ifield
            endif
         enddo
c         r0d=r0(iuse)/radtodeg
c         d0d=d0(iuse)/radtodeg
c         jstart=1
         jstart=i+1
         do j=jstart,n
            r2d=r(j)/radtodeg
            ...
            if(diff.le.bsmax) then
               call qags(func2,0.,z1,epsabs,epsrel,ss2,abserr,
     $              neval,ier,limit,lenw,last,iwork,work)
               dm1=cee/xh*ss2
               call qags(func2,0.,z2,epsabs,epsrel,ss2,abserr,
     $              neval,ier,limit,lenw,last,iwork,work)
               dm2=cee/xh*ss2
c               dma=(dm1+dm2)/2.

c               th1=acos(sin(d1d)*sin(d0d)+
c     $              cos(d1d)*cos(d0d)*cos(r1d-r0d))
c               th2=acos(sin(d2d)*sin(d0d)+
c     $              cos(d2d)*cos(d0d)*cos(r2d-r0d))
c               d1b=th1*radtodeg*dm1/206265.*60.*60.
c               d2b=th2*radtodeg*dm2/206265.*60.*60.
c               diff2=abs(d1b-d2b)

               rd1=cdec*(r(i)-r0(iuse))*dm1/206265.*60.*60.
               rd2=cdec*(r(j)-r0(iuse))*dm2/206265.*60.*60.
               dd1=(d(i)-d0(iuse))*dm1/206265.*60.*60.
               dd2=(d(j)-d0(iuse))*dm2/206265.*60.*60.
               delr=rd1-rd2
               deld=dd1-dd2
               pdiff=sqrt(delr*delr+deld*deld)
c               pdiff0=sqrt(diff)*dma/206265.*60.*60.
               zdiff=dm1-dm2
c               rdiff0=sqrt(pdiff0*pdiff0+zdiff*zdiff)
               rdiff=sqrt(pdiff*pdiff+zdiff*zdiff)
               ang=radtodeg*acos(pdiff/rdiff)

               call xlinint(diff,nbin,bs,xi,x0)
               ibin=nint(x0)
               ncorr(ibin)=ncorr(ibin)+1
               if(rdiff.le.rdmax.and.rdiff.ge.rdmin) then
                  call xlinint(rdiff,nbin,rd,xi,x0)
                  ibin=nint(x0)
                  nrcorr(ibin)=nrcorr(ibin)+1
                  corr(ibin)=corr(ibin)+w1*w2
                  if(ang.le.30.) then
                     if(pdiff.le.rdmax.and.pdiff.ge.rdmin) then
                        call xlinint(pdiff,nbin,rd,xi,x0)
                        ibin=nint(x0)
                        corrdm(ibin)=corrdm(ibin)+w1*w2
                     endif
                  endif
                  if(ang.ge.60.) then
                     if(zdiff.le.rdmax.and.zdiff.ge.rdmin) then
                        call xlinint(zdiff,nbin,rd,xi,x0)
                        ibin=nint(x0)
                        corrdh(ibin)=corrdh(ibin)+w1*w2
                     endif
                  endif
               endif
            endif
      ...
      do i=1,nbin
c         write(11,*) rd(i),nrcorr(i),b(i),ncorr(i)
c         write(11,*) rd(i),corr(i),b(i),ncorr(i)
         write(11,*) rd(i),corr(i),corrdm(i),corrdh(i)
