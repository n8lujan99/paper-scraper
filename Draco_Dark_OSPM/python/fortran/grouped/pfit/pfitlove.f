      ...
      real yl(nmax),yh(nmax),xp(nmax),yp(nmax)
      integer ia(6)
      character file1*40
      data big/1.e30/
...
c         if(if.eq.1) call kgenv(nl,v,yh)
         call kgenv(nl,v,yh)
c         call pglabel('','',file1(1:nfile))
         call pglabel("Velocity (km s\U-1\D)","Relative Light","")
         np=0
         do i=1,nl
            np=np+1
            xp(np)=v(i)
...
c         call pgsci(if)
         ...
         call pgslw(2)
c         if(if.eq.1) then
c            call pgline(nl,v,yl)
c            call pgline(nl,v,yh)
c         endif
         call pgsls(1)
         amp=a(1)
         vel=a(2)
...
c      call pgenv(xmin-xbit,xmax+xbit,-0.0005,ymax+ybit,0,0)
      call pgenv(xmin-xbit,xmax+xbit,-0.00015,ymax+ybit,0,0)
