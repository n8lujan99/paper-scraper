...
def get_sky(spectra_stack: np.ndarray, method: str = "biweight") -> Tuple[np.ndarray, np.ndarray]:
    """
    Collapse a stack of spectra (N x M) into a sky estimate per pixel and its scatter.

    method="biweight": use robust biweight location/scale via LOC/Scale
    otherwise:        fallback to median and 1.4826*MAD
    """
    spectra_stack = np.asarray(spectra_stack, dtype=float)
    if spectra_stack.ndim != 2:
        raise ValueError("spectra_stack must be a 2D array shaped (N_spectra, N_pixels).")
