...
def pair_counts(ra_deg=None, dec_deg=None, bins_deg=None, weights: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    wl  = np.ones(vec.shape[0]) if weights is None else np.asarray(weights, float)[m]
    tree = cKDTree(vec)
    bins = _ensure_bins(bins_deg)
    return _hist_angles_from_tree(vec, vec, tree, bins, weights_left=wl, weights_right=wl, drop_self=True)

def pair_counts_cross(ra1_deg=None, dec1_deg=None, ra2_deg=None, dec2_deg=None, bins_deg=None,weights_left: Optional[np.ndarray] = None, weights_right: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    wL  = np.ones(v1.shape[0]) if weights_left  is None else np.asarray(weights_left,  float)[m1]
    wR  = np.ones(v2.shape[0]) if weights_right is None else np.asarray(weights_right, float)[m2]
    tree2 = cKDTree(v2)
    bins  = _ensure_bins(bins_deg)
    return _hist_angles_from_tree(v1, v2, tree2, bins, weights_left=wL, weights_right=wR, drop_self=False)
...
def wtheta_landy_szalay(raD=None, decD=None, raR=None, decR=None, bins_deg=None, wD=None, wR=None):
...
NR = max(30000, oversample * ND)
RA_R, DEC_R = make_randoms(ra0_deg=ra0, dec0_deg=dec0, radius_deg=RR_cap_deg, n=NR, seed=42)

centers, w = wtheta_landy_szalay(raD=raD, decD=decD, raR=RA_R, decR=DEC_R, bins_deg=bins_deg)
# drop the first bin 
centers_plot, w_plot = centers[1:], w[1:]
...
plt.semilogx(centers_plot, w_plot, marker="o", lw=1)
plt.axhline(0, ls="--")
plt.xlabel(r"$\theta$ [deg]  (log)")
plt.ylabel(r"$w(\theta)$")
plt.title("Draco two-point angular correlation (small scales)")
plt.tight_layout()
plt.show()
...
def xlinint(xp, x, y):
    ...
    return float(np.interp(float(xp), x, y, left=y[0], right=y[-1]))
